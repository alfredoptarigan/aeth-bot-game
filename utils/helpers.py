...
def get_dice_status(user_id):
    ...
    (current_exp, current_level, current_currency, last_daily_str, atk, spd, def_stat, dex, crit, mdmg, hp, mp, slots, equipped_weapon, equipped_armor, current_rolls, last_reset_str, last_fight_time, hunt_count, hunt_reset, max_inventory) = user_data

    today = datetime.date.today().strftime('%Y-%m-%d')
    if last_reset_str != today:
        current_rolls = 0
        last_reset_str = today
        update_full_user_data(user_id, current_exp, current_level, current_currency, last_daily_str, atk, spd, def_stat, dex, crit, mdmg, hp, mp, slots, equipped_weapon, equipped_armor, current_rolls, last_reset_str, last_fight_time, hunt_count, hunt_reset, max_inventory)
